...
def skip_event(events, event):
    ...
    #   The check says, if the format is Singles/Doubles, but Singles/Doubles isn't in the title of the event, skip it - Skip if Squad is in the name as well
    not_found = 1
    for format_num in smash_formats:
        if((smash_formats[format_num] in events[event].event_name or 'Ultimate 1v1' in events[event].event_name) and not('Squad' in events[event].event_name)):
            not_found = 0
            break;
...
def update_master_file(master_filename, slug, tournament_name, tournament_dates, event):
    ...
    if(not check):
        try:
            ...
            master.close()
        except UnicodeEncodeError: # Japanese tournament character name
            print("Failed to write to the master tournament file. Please manually input this yourself.")
...
def write_set_data(sets_file, event, supress):
    ...
    if(event.format == "Doubles"):
        ...
        doubles = 0
        f.write("P1,P2,set winner,P1Score,P2Score,Round,roundDivision\n")
    ...
    for group in event.groups:
        ...
        if(result_data["entities"].get("sets",0)):
            pass
        else:
            continue
        for _set in result_data["entities"]["sets"]:
            #print(_set.keys()) - dict_keys(['fullRoundText', 'hasPlaceholder', 'entrant1CharacterIds', 'createdAt', 'midRoundText', 'entrant2PrereqType', 'loserId', 'adminViewedBy', 'entrant1Present', 'phaseGroupId', 'round', 'wPlacement', 'entrant2CharacterIds', 'durationSeconds', 'progressionCount', 'smashggUrl', 'displayRound', 'adminViewedAt', 'roundDivision', 'games', 'updatedAt', 'eventType', 'unreachable', 'callOrder', 'updatedAtMicro', 'originalRound', 'entrant1Score', 'stationId', 'videogameId', 'entrant1Id', 'tasksPruned', 'streamId', 'slots', 'startedAt', 'setGamesType', 'adminMessagedBy', 'entrant2Score', 'entrant1PrereqCondition', 'state', 'type', 'bracketId', 'progressionPlacement', 'totalGames', 'winnerId', 'startAt', 'lEligiblePhaseId', 'phaseId', 'shortRoundText', 'wProgressionSeedId', 'modRequestedBy', 'fbUrl', 'expand', 'entrant2Id', 'modRequestedAt', 'wOverallPlacement', 'bestOf', 'entrant2Present', 'id', 'adminMessagedAt', 'entrant2PrereqId', 'isTiebreak', 'crewPlayerCount', 'entrant1PrereqId', 'isLast', 'lProgressionSeedId', 'identifier', 'lOverallPlacement', 'entrant2PrereqCondition', 'wEligiblePhaseId', 'eventId', 'isGF', 'images', 'completedAt', 'subState', 'lPlacement', 'entrant1PrereqType', 'vodUrl'])
            p1 = _set["entrant1Id"]
            p2 = _set["entrant2Id"]
            ...
            try:
                f.write(event.entrants[p1] + ',' + event.entrants[p2] + ',' + str(result) + "," + str(p1_score) + "," + str(p2_score) + "," + str(_set['fullRoundText']) + "," + str(_set['roundDivision']) + '\n')
            except UnicodeEncodeError: # due to Japanese chars, we have this monstrosity.
                try: # tranlate p1 if it has an encode error
                    dummy = event.entrants[p1].encode('ascii')
                    dummy_p1 = event.entrants[p1]
                except UnicodeEncodeError:
                    try: # if the translator fails, give Random player name
                        dummy_p1 = translator.translate(event.entrants[p1], src='ja', dest='en').text
                    except:
                        dummy_p1 = 'Random_Japanese_Player'
                try:
                    dummy = event.entrants[p2].encode('ascii')
                    dummy_p2 = event.entrants[p2]
                except UnicodeEncodeError:
                    try:
                        dummy_p2 = translator.translate(event.entrants[p2], src='ja', dest='en').text
                    except:
                        dummy_p2 = 'Random_Japanese_Player'
                try: # try to write again, sometimes google translate fails
                    f.write(dummy_p1 + ',' + dummy_p2 + ',' + str(result) + "," + str(p1_score) + "," + str(p2_score) + "," + str(_set['fullRoundText']) + "," + str(_set['roundDivision']) + '\n')
                except UnicodeEncodeError: # Here we change an player name to Random_Japanese_Player due to issues
                    try:
                        dummy = dummy_p1.encode('ascii')
                    except UnicodeEncodeError:
                        dummy_p1 = 'Random_Japanese_Player'
                    try:
                        dummy = dummy_p2.encode('ascii')
                    except UnicodeEncodeError:
                        dummy_p2 = 'Random_Japanese_Player'
                    f.write(dummy_p1 + ',' + dummy_p2 + ',' + str(result) + "," + str(p1_score) + "," + str(p2_score) + "," + str(_set['fullRoundText']) + "," + str(_set['roundDivision']) + '\n')
    if(not supress):
    ...
    def add_entrants(self, entrants):
        for entrant in entrants:
            if(self.format == "Doubles"):
                ...
                try: # In some cases a tourny won't have placements, return 0
                    self.placings[entrant["id"]] = entrant["finalPlacement"]
                except:
                    self.placings[entrant["id"]] = 0
            else:
